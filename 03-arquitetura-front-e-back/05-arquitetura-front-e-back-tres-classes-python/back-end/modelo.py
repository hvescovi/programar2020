...
class ExameRealizado(db.Model):
    ...
    # construtor para ASSEGURAR que vai haver pessoa no exame!
    def __init__(self, data, nome, resultado, pessoa):
        if pessoa is None:
            raise Exception('Não pode haver exame sem paciente!')
        # atribuição dos valores aos campos
        self.data = data
        self.nome = nome
        self.resultado = resultado 
        self.pessoa = pessoa
        # nomes dos parâmetros do construtor devem ser iguais aos nomes dos campos,
        # pois se usa a forma 'campo=valor' para instanciar o objeto
